...
void GLInjectInput::InputThread() {
	try {
		...
		while(!m_should_stop) {
			...
			const uint8_t *image_data[1] = {(uint8_t*) data};
			int image_stride[1] = {stride};
			PushVideoFrame(width, height, image_data, image_stride, AV_PIX_FMT_BGRA, SWS_CS_DEFAULT, timestamp);
