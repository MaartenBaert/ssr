...
void GLInjectLauncher::Init() {
	...
	if(m_shm_main_id == -1) {
		Logger::LogError("[GLInjectLauncher::Init] " + Logger::tr("Error: Can't get shared memory!"));
		throw GLInjectException();
	}
	m_shm_main_ptr = (char*) shmat(m_shm_main_id, NULL, SHM_RND);
	if(m_shm_main_ptr == (char*) -1) {
		Logger::LogError("[GLInjectLauncher::Init] " + Logger::tr("Error: Can't attach to shared memory!"));
		throw GLInjectException();
	}
	...
	for(unsigned int i = 0; i < RING_BUFFER_SIZE; ++i) {
		...
		if(m_shm_frames.back().m_id == -1) {
			Logger::LogError("[GLInjectLauncher::Init] " + Logger::tr("Error: Can't get frame shared memory!"));
			throw GLInjectException();
		}
		m_shm_frames.back().m_shm_ptr = (char*) shmat(m_shm_frames.back().m_id, NULL, SHM_RND);
		if(m_shm_frames.back().m_shm_ptr == (char*) -1) {
			Logger::LogError("[GLInjectLauncher::Init] " + Logger::tr("Error: Can't attach to frame shared memory!"));
			throw GLInjectException();
		}
	...
	Logger::LogInfo("[GLInjectLauncher::Init] " + Logger::tr("Full command") + ": " + full_command);
	...
	if(m_run_command) {
		...
		if(!QProcess::startDetached("/bin/sh", args, m_working_directory)) {
			Logger::LogError("[GLInjectLauncher::Init] " + Logger::tr("Error: Can't run command!"));
			throw GLInjectException();
		}
	}
