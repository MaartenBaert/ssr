...
void GLInjectInput::Init() {
	...
	// initialize shared data
	{
		...
		lock->m_stream_listener.reset(new SSRVideoStreamListener(m_channel.toStdString(), m_relax_permissions));
	}
...
void GLInjectInput::StreamRemoveCallback(const SSRVideoStream& stream, size_t pos, void* userdata) {
	...
	if(lock->m_stream_reader != NULL) {
		if(lock->m_stream_reader->GetStream() == stream) {
			lock->m_stream_reader.reset();
			auto &streams = lock->m_stream_listener->GetStreams();
...
void GLInjectInput::InputThread() {
	try {
		...
		int64_t next_listener_update = hrt_time_micro(); //TODO// needed?
		...
		while(!m_should_stop) {
			...
			void *data;
			{
				...
				if(hrt_time_micro() >= next_listener_update) {
					lock->m_stream_listener->HandleChanges(&StreamAddCallback, &StreamRemoveCallback, this);
					next_listener_update = hrt_time_micro() + 200000;
				}
